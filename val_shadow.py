from dataloader.spacedataset import Spacedateset,Spaceshadowset
import os
...
def parse_args() -> Namespace:
    ...
    parser.add_argument('--n', type=str,default="shadow",choices=["PEnet", "semnet","plane"],help='choose a model')
    ...
    parser.add_argument('-b',
                        '--batch-size',default=1,type=int,help='mini-batch size (default: 1)')
    ...
    parser.add_argument('--pretrain_backbone',default="./checkpoints/semnet_backbone_best.pth",help="backbone pretained")
    ...
    for i, data in enumerate(dataloader):
        ...
        if iter_mode == 'train':
            ...
            if i % args.print_freq == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                      'Loss {loss:.4f} rgb_loss {rgb_loss:.4f}'.format(epoch, i, len(dataloader), loss=loss.item(),
                                                                       rgb_loss=loss_2.item()))
...
def main():
    ...
    args.shadow = True
    args.result = os.path.join('.', 'val_shadow')
    args.val = True
    val_set = Spaceshadowset(args)
    logger = Logger(args)
    args.prefill = True
    args.trained_model = "./checkpoints/shadow_CSPN_best.pth"
    ...
    is_best, avg = iterate('val', args, val_loader, net, optimizer=None, epoch=0, device=device, Loss=Loss, logger=logger)
